        ...
        <div class="column is-4 animated fadeIn"
          ...
          <div class="card is-shady card-equal-height">
            ...
            <div class="card-image has-text-centered">
              <img :src="twitch.data.logo">
            </div>
            <div class="card-content">
              <div class="content">
                <h4 class="has-text-centered">  {{ twitch.data.display_name }}</h4>
                <h5 class="has-text-centered">Bio: {{ twitch.data.bio }}</h5>
                <h5 class="has-text-centered">Date Created: {{ moment(twitch.data.created_at).format('YYYY-MM-DD') }}</h5>
              </div>
            </div>
...
<script>
  ...
  export default {
    ...
    computed:{
      twitch(){
        return this.$store.getters['twitch/getProfile']
      },
    },
    created(){
    ...
    methods:{
      ...
      linkTwitch() {
        window.location = process.env.TWITCH_URL + this.token
      },

      unlinkTwitch (context) {
        this.$store.dispatch('twitch/unlinkTwitch', this.token)
